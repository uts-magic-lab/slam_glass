...
inline double ScanMatcher::icpStep(OrientedPoint & pret, const ScanMatcherMap& map, const OrientedPoint& p, const double* readings) const{
	...
	for (const double* r=readings+m_initialBeamsSkip; r<readings+m_laserBeams; r++, angle++){
		...
		if (skip) continue;
		d = *r;
		if (isnan(d)|| d>m_usableRange||d==0.0) continue;
		Point phit=lp;
		phit.x+=d*cos(lp.theta+*angle);
		phit.y+=d*sin(lp.theta+*angle);
		IntPoint iphit=map.world2map(phit);
		Point pfree=lp;
		pfree.x+=(d-map.getDelta()*freeDelta)*cos(lp.theta+*angle);
		pfree.y+=(d-map.getDelta()*freeDelta)*sin(lp.theta+*angle);
 		pfree=pfree-phit;
...
inline unsigned int ScanMatcher::likelihoodAndScore(double& s, double& l, const ScanMatcherMap& map, const OrientedPoint& p, const double* readings) const{
    ...
    if (skip) continue;
    d = *r;
		if (isnan(d) || d>m_usableRange) continue;
		Point phit=lp;
		phit.x+=d*cos(lp.theta+*angle);
		phit.y+=d*sin(lp.theta+*angle);
		IntPoint iphit=map.world2map(phit);
		Point pfree=lp;
		pfree.x+=(d-freeDelta)*cos(lp.theta+*angle);
		pfree.y+=(d-freeDelta)*sin(lp.theta+*angle);
		pfree=pfree-phit;
		IntPoint ipfree=map.world2map(pfree);
		bool found=false;
		Point bestMu(0.,0.);
